...
class FixedDenseConnections(Connections):
    ...
    def __init__(
        ...
        reuse_weight=1.0,
        class_balance_change_fraction=0.25,
        ...
        **kwargs
        ...
        self.class_balance_change_fraction = class_balance_change_fraction
        ...
        if self.init_method == "random" and self.topology_seed is None:
            ...
            result = generate_dense_topology(
                ...
                class_balance_change_fraction=(
                    self.class_balance_change_fraction
                ),
...
class FixedConvConnections(Connections):
    ...
    def _init_connections(self):
        ...
        if self.strategy == "random":
            ...
            if self.channel_group_size != 2:
                raise ValueError(
                    "semantic_channel_spatial_hybrid currently requires "
                    "channel_group_size=2"
                )
            if self.lut_rank != 2:
                raise NotImplementedError(
                    "semantic_channel_spatial_hybrid currently supports "
                    "rank-2 LUTs only"
                )
            # Keep the frozen V4 channel construction exactly intact. The new
            # domain adapter changes only how those channels are assigned to
            # receptive-field offsets and bottom-level tree gates.
            channel_topology = generate_dense_topology(
                in_dim=self.channels,
                out_dim=self.num_kernels,
                lut_rank=2,
            ...
            kernels = self._apply_channel_spatial_pairing(
                kernels,
                channel_pairs=self.channel_pairs,
            )
            if self.input_channel_ancestry is not None:
    ...
    def _apply_channel_spatial_pairing(self, kernels, channel_pairs):
        """Pair selected channels across immutable spatial leaf coordinates.

        ``kernels`` is generated by the frozen V4 spatial sampler. This adapter
        changes only its channel column: each bottom-level rank-2 LUT receives
        one leaf from each selected channel, and rank orientation alternates to
        preserve exact per-kernel channel degree. Spatial coordinates remain
        bit-identical to V4, as required by the CoverageDLGN specification.
        """
        if channel_pairs.ndim != 2:
            raise ValueError("channel_pairs must have shape (channels, kernels)")
        if channel_pairs.shape != (self.channel_group_size, self.num_kernels):
            raise ValueError(
                "channel_pairs shape must be "
                f"({self.channel_group_size}, {self.num_kernels}), got "
                f"{tuple(channel_pairs.shape)}"
            )

        if kernels.ndim != 4:
            raise ValueError(
                "kernels must have shape "
                "(lut_rank, kernels, leaves, spatial+channel)"
            )
        if kernels.shape[:2] != (self.lut_rank, self.num_kernels):
            raise ValueError(
                "kernel tensor prefix must be "
                f"({self.lut_rank}, {self.num_kernels}), got "
                f"{tuple(kernels.shape[:2])}"
            )
        if self.lut_rank != channel_pairs.shape[0]:
            raise ValueError(
                "channel-spatial pairing requires one selected channel per "
                "LUT input rank"
            )
        paired = kernels.clone()
        leaf_count = paired.shape[2]
        leaves = torch.arange(leaf_count, device=self.device)
        for kernel in range(self.num_kernels):
            orientation = (leaves + kernel) % self.lut_rank
            for rank in range(self.lut_rank):
                paired[rank, kernel, :, -1] = channel_pairs[
                    (orientation + rank) % self.lut_rank,
                    kernel,
                ]
        return paired

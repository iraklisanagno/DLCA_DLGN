#!/usr/bin/env python3
"""Audit and summarize the conditional one-seed CIFAR-10 M Mommen run."""
...
QUEUE_PATH = ROOT / "queues" / "table2_m_mommen_final.json"
QUEUE_SUMMARY = ROOT / "logs" / "table2_m_mommen_final" / "queue_summary.json"
JSON_PATH = ROOT / "summary" / "table2_m_mommen_final.json"
CSV_PATH = ROOT / "summary" / "table2_m_mommen_final.csv"
...
def main() -> None:
    ...
    if audit["failed"]:
        raise RuntimeError(f"Mommen M queue failed: {audit['failed']}")
    if len(queue["entries"]) != 1:
        raise RuntimeError("conditional Mommen M policy requires exactly one run")
    ...
    payload = {
        "phase": queue["phase"],
        "policy": (
            "one full seed; no promotion because it is not competitive "
            "with five-seed CoverageDLGN V3"
        ),
        "provenance": "TRIED",
    ...
    print(JSON_PATH)
    print(
        f"validation={100 * row['best_validation_hard_accuracy']:.3f}% "
        f"test_set_used={test_set_used}"
    )
    if test_set_used:

#!/usr/bin/env python3
"""Aggregate and select candidates from the Table 1 Fashion-MNIST screen."""
...
def main() -> None:
    ...
    for summary_path in sorted(
        RESULT_ROOT.glob("screen_table1_fashion_*/run_summary.json")
    ):
        ...
        row = {
            ...
            "config": str(
                Path("experiments/coverage_dlgn/configs/table1_screen_fashion")
                / f"{run_dir.name}.json"
            ),
        }
        rows.append(row)
    ...
    csv_path = SUMMARY_ROOT / "table1_fashion_screen.csv"
    ...
    json_path = SUMMARY_ROOT / "table1_fashion_screen.json"

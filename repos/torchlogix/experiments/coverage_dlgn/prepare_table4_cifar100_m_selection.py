#!/usr/bin/env python3
"""Generate the paired three-seed CIFAR-100 M 20K selection queue."""
...
SCREEN = ROOT / "summary" / "table4_cifar100_m_screen.json"
CONFIG_DIR = ROOT / "configs" / "table4_cifar100_m_selection"
QUEUE_PATH = ROOT / "queues" / "table4_cifar100_m_selection.json"
...
def main() -> None:
    ...
    if not screen["advance_to_20k"]:
        raise RuntimeError("M diagnostic screen did not authorize selection")
    winner = next(
    ...
    generated = []
    for seed in (0, 1, 2):
        random_name = f"select_table4_cifar100_m_random_seed{seed}"
        ...
        v3_name = (
            f"select_table4_cifar100_m_v3_{winner['candidate']}_seed{seed}"
        )
    ...
    payload = {
        "phase": "table4_cifar100_m_selection",

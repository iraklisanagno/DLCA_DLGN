#!/usr/bin/env python3
"""Evaluate frozen 6x64K CIFAR-100 checkpoints exactly once on test."""
...
SUMMARY_PATH = ROOT / "summary" / "table4_cifar100_deep_final.json"
LOG_DIR = ROOT / "logs" / "table4_cifar100_deep_final" / "test"
...
def main() -> int:
    ...
    run_dirs = [Path(entry["output"]) for entry in queue["entries"]]
    if len(run_dirs) != 6 or len(set(run_dirs)) != 6:
        raise RuntimeError(f"expected six unique runs, got {len(run_dirs)}")
    missing = [

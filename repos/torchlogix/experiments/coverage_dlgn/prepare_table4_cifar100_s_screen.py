#!/usr/bin/env python3
"""Generate the frozen one-seed dense CIFAR-100 S topology screen."""
...
CONFIG_DIR = ROOT / "configs" / "table4_cifar100_s_screen"
QUEUE_PATH = ROOT / "queues" / "table4_cifar100_s_screen.json"
RESULT_ROOT = Path("experiments/coverage_dlgn/results")

# Existing frozen V3 controls only. The topology algorithm and raw gate
# parameterization are identical in every candidate.
COVERAGE_CANDIDATES = (
...
def main() -> None:
    ...
    random_name = "screen_table4_cifar100_s_random_seed0"
    ...
    for label, pool, swap, novelty in COVERAGE_CANDIDATES:
        name = f"screen_table4_cifar100_s_v3_{label}_seed0"
        config = common(name)
        config.update({
    ...
    payload = {
        "phase": "table4_cifar100_s_screen",
        "purpose": "one-seed 5K validation topology screen; not final",
        "selection_metric": "best hardened validation accuracy",
        "heldout_test_used": False,
        "architecture": {
            "name": "DlgnCifar100BitLogicS",
            "depth": 2,
            "width": 4_000,
            "gate_count": 8_000,
            "lut_rank": 2,

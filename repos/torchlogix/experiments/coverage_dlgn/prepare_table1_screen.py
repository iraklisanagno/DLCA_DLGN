#!/usr/bin/env python3
"""Generate the locked one-seed Table 1 MNIST screening configurations."""
...
CONFIG_DIR = ROOT / "configs" / "table1_screen_mnist"
QUEUE_PATH = ROOT / "queues" / "table1_screen_mnist.json"
...
def fixed_random() -> tuple[str, dict]:
    name = "screen_table1_mnist_random_48k_seed0"
    config = common(name)
    config.update({
        "architecture": "DlgnMnistPaperSmall",
        "learning_rate": 0.01,
        "connections": "fixed",
...
def coverage_configs() -> list[tuple[str, dict]]:
    ...
    for label, pool, swap, novelty, parametrization in candidates:
        name = f"screen_table1_mnist_v3_{label}_48k_seed0"
        config = common(name)
        config.update({
            "architecture": "DlgnMnistPaperSmall",
            "learning_rate": 0.01,
...
def learnable_architecture(depth: int) -> str:
    return {
        2: "DlgnMnistLearnable48kDepth2",
        3: "DlgnMnistLearnable48kDepth3",
        6: "DlgnMnistPaperSmallLearnable",
    }[depth]
...
def mommen_configs() -> list[tuple[str, dict]]:
    ...
    for candidate_count in (8, 16):
        for depth in (2, 3, 6):
            name = (
                f"screen_table1_mnist_mommen_nc{candidate_count}_"
...
def lilogic_configs() -> list[tuple[str, dict]]:
    configs = []
    for depth in (2, 3, 6):
        for group_sum_temperature in (10, 15, 20):
            name = (
                f"screen_table1_mnist_lilogic_top32_depth{depth}_"
...
def bitlogic_config() -> tuple[str, dict]:
    name = "screen_table1_mnist_bitlogic_48k_seed0"
    config = common(name, batch_size=128)
    config.update({
        "architecture": "DlgnMnistBitLogic48k",
        "learning_rate": 0.01,
        "weight_decay": 0.0,

...
efficient machine learning by learning directly with the primitive operations
used in digital hardware. Rather than relying on multiply-accumulate
operations, a DLGN is composed of Boolean gates such as \textsc{and},
\textsc{or}, and \textsc{xor}. During training, these discrete operations are
...
particularly attractive for resource-constrained and hardware-oriented
machine learning.

A fundamental characteristic of this architecture, however, is its sparse
connectivity. Each conventional DLGN gate has only two inputs. In the original
formulation, these two predecessors are selected before training and remain
...
signals are combined, but not \emph{which} signals meet at a gate. If two
...
Recent work increasingly supports this view of connectivity as an independent
design dimension. BitLogic, for example, separates connectivity from the gate
parameterization, fan-in, input encoding, and output head when studying the
design space of LUT- and logic-native neural networks \cite{buehrer2026}.
Other approaches go further and optimize the predecessors themselves during
training \cite{mommen2025,fojcik2026}. Learned connectivity can recover
connections that were absent from the original graph and can therefore improve
accuracy or reduce the number of required gates. This flexibility, however, is
not free. During training, each gate must consider multiple candidate signals,
...
Before designing such a method, we ask whether a simpler property---balanced
predecessor usage---is already sufficient. As a motivational experiment, we
consider a dense CIFAR-10 network whose deeper connectivity is kept fixed and
structured. We then randomize only the first-layer predecessor identities
while preserving, for every input slot, exactly how often each predecessor is
used. The resulting graph therefore maintains the same predecessor degrees,
but forms different input pairs. This seemingly small change reduces hardened
validation accuracy by $4.49$ percentage points, with a paired $95\%$
confidence interval of $[3.90,5.09]$ over three seeds. Since the native dense
random construction already provides broadly balanced fan-out, this experiment
shows that \emph{which signals are paired matters beyond how frequently each
signal is used}. It motivates exploiting signal identity when constructing the
graph rather than treating predecessor indices as interchangeable.

Based on this observation, we propose \textbf{LogicConnect}, an offline
connectivity construction for dense and convolutional DLGNs. LogicConnect
first introduces structure where signal identity is most explicit. For dense
image inputs, it forms semantic first-layer pairs according to relationships
among the encoded image signals rather than arbitrary input indices. In
subsequent layers, LogicConnect organizes predecessor interactions using
regular multiscale matchings. Complete matching stages are selected primarily
to balance predecessor usage, while an ancestry-overlap criterion
distinguishes alternatives with equivalent degree balance. Once constructed,
the graph is frozen and conventional DLGN training learns only the gate
functions.

The same principle is applied to convolutional DLGNs without replacing their
convolutional execution model. LogicConnect modifies the channel pairs
available to the shared logic trees while retaining the existing spatial
sampler and parameter sharing of the convolutional architecture
\cite{petersen2024}. The classifier is then constructed using the same fixed
multiscale connectivity principle employed by the dense network. Thus,
LogicConnect neither introduces a new inference operator nor increases the
declared depth, width, or gate budget. Instead, it moves connectivity design
offline, seeking a more useful fixed graph without creating an additional
trainable routing problem.

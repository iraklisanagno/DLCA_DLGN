...
dense CIFAR-10 DLGN with 512K gates.
We compare first-layer pairing constructed using spatial, channel, and encoding relationships against a randomized control derived from the same graph.
The control
...
differ only in which inputs are paired.
All deeper connections remain unchanged. Both graphs are constructed before training and trained separately for $20,000$ updates using the same data split, paired initialization, and training configuration.
\input{tables/motivation.tex}
...
$4.49\%$. Hence, balanced predecessor usage alone does not
determine connectivity quality: the specific signals combined by each gate
also matter. 
Exhaustively evaluating candidate pairs, however, becomes costly as the network grows. With $N$ available predecessors, each two-input gate admits

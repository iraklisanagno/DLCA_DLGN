#!/usr/bin/env python3
"""Audit and summarize the dense CIFAR-100 S topology screen."""
...
QUEUE_PATH = ROOT / "queues" / "table4_cifar100_s_screen.json"
QUEUE_SUMMARY = (
    ROOT / "logs" / "table4_cifar100_s_screen" / "queue_summary.json"
)
JSON_PATH = ROOT / "summary" / "table4_cifar100_s_screen.json"
CSV_PATH = ROOT / "summary" / "table4_cifar100_s_screen.csv"
...
def main() -> None:
    ...
    if audit["failed"]:
        raise RuntimeError(f"CIFAR-100 S screen failed: {audit['failed']}")
    ...
    random_row = next(row for row in rows if row["family"] == "random")
    winner = max(
    ...
    payload = {
        ...
        "raw_random": random_row["name"],
        "selected_raw_v3": winner["name"],
        "screen_gain_pp": 100 * (
            winner["best_validation_hard_accuracy"]
            - random_row["best_validation_hard_accuracy"]
        ),
        "advance_to_20k": [random_row["name"], winner["name"]],
    ...
    print(
        ...
        f"{100 * winner['best_validation_hard_accuracy']:.3f}% "
        f"screen_gain={payload['screen_gain_pp']:+.3f} pp"
    )

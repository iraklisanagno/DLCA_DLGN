...
run_one() {
    local gpu=$1
    local seed=$2
    local output="experiments/coverage_dlgn/results/paper_fashion_mnist_small_semantic_balanced_v3_seed${seed}"
    if [[ -e "$output" ]]; then
    ...
    env DATASET_PATH=/tmp/torchlogix-datasets CUDA_VISIBLE_DEVICES="$gpu" \
        ...
        --config experiments/coverage_dlgn/configs/paper_fashion_mnist_small_semantic_balanced_v3_seed0.json \

...
def layer_analysis(indices, luts, source_ids):
    ...
    from .construction import validate_indices
    for depth, (parents, tables) in enumerate(zip(indices, luts)):
        parents = validate_indices(parents, len(support))
...
def freeze_selection():
    ...
    value = dict(scope="checkpoint_only_no_dataset_no_inference", rows=selected,
        sources={name: sha(ROOT / name) for name in ["functional_support.py", "construction.py", "common.py"]},
    ...
    write_json(ROOT / "analysis_selection.json", value)
    return value


def analyze():
    frozen = read(ROOT / "analysis_selection.json")
    verify_incumbent()
    ...
    report = dict(status="complete", selection_sha256=sha(ROOT / "analysis_selection.json"),
